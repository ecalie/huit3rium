...
public class Projet {
	...
	public void afficherFenetreReponses() {
		for (Niveau niv : Niveau.values()) {
			FicheReponse fr = null;
			if (ficheReponse.get(niv) == null) {
				fr = new FicheReponse(this, niv);
	 ...
	 *
	 * @param numero Le numéro du jeune à ajouter
	 **/
	...
	public void enregistrer() {
		...
		for (int ind = 1; ind <= nbCircuit; ind++) {
			...
			for (int ii = 1; ii <= this.nbOrientation; ii++) {
				...
				for (Niveau niv : Niveau.values()) {
					row.createCell(col);
					System.out.println(ind + "  " + ii + "  " + this.reponses.get(niv).get("orientation" + ind + "_" + ii));
